            ...
            <tbody className="divide-y">
              {bookings.map((b) => (
                <tr key={b.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 font-medium">{b.agent_name || b.agent_id}</td>
                  <td className="px-4 py-3">{b.type}</td>
                  <td className="px-4 py-3">{b.reference_id || "-"}</td>
                  <td className="px-4 py-3">{b.adults}</td>
                  <td className="px-4 py-3">PKR {Number(b.total_amount).toLocaleString()}</td>
                  ...
                  <td className="px-4 py-3">{b.created_at ? new Date(b.created_at).toLocaleDateString() : "-"}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => handleDelete(b.id)} className="text-red-500 hover:text-red-700"><Trash2 size={16} /></button>
                  </td>
                </tr>
              ))}
              {bookings.length === 0 && <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-500">No bookings found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
