...
export default async function AllPackageBookingsPage() {
  ...
  const packageBookings = bookings.filter((b) => b.type === "package" || b.type === "umrah");

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">All Package Bookings</h1>
              ...
              <tbody>
                {packageBookings.map((b) => (
                  <tr key={b.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3">{packages.find((p) => p.id === b.package_id)?.title || "N/A"}</td>
